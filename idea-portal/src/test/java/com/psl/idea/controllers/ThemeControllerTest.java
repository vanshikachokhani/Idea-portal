...
public class ThemeControllerTest {
	...
	private ThemeFiles themeFile = new ThemeFiles("1.1_Test.txt", "application/text", t);
	private MultipartFile[] files = {};
	private ThemeFiles[] themeFiles = {themeFile};
...
//	String jwtToken = "eyJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE2MjY4NjA5ODEsImV4cCI6MTYyNjg2ODE4MSwidXNlcklkIjo0LCJlbWFpbElkIjoicmF0aGlyb2hhbjhAZ21haWwuY29tIiwibmFtZSI6IlJvaGFuIFJhdGhpIiwicHJpdmlsZWdlIjp7InByaXZpbGVnZUlkIjoxLCJwcml2aWxlZ2UiOiJDbGllbnQgUGFydG5lciJ9fQ.NOCopJfwqEaIZA9S9x_BeNSANjAd95g8vFfe2m_bI3M";
	...
	MockMultipartFile mockThemeFile = new MockMultipartFile("files", "Test.png", MediaType.IMAGE_PNG_VALUE, new byte[2]);
	...
	public void createThemeTest() throws Exception {
		...
		mockMvc.perform(multipart("/api/loggedin/themes").file(mockThemeFile).param("categoryId", "1").requestAttr("userId", 1).flashAttr("theme", t)
		...
		mockMvc.perform(multipart("/api/loggedin/themes").file(mockThemeFile).param("categoryId", "1").requestAttr("userId", 2).flashAttr("theme", t)
	...
	public void createIdeaTest() throws Exception {
		when(ideaService.createIdea(any(Long.class), any(Idea.class), any(MultipartFile[].class))).thenReturn(i, (Idea) null);
		
		when(usersUtil.getPrivilegeIdFromRequest(any())).thenReturn(user2.getPrivilege().getPrivilegeId(), user2.getPrivilege().getPrivilegeId(), user1.getPrivilege().getPrivilegeId());
		when(userService.getUserByUserId(2)).thenReturn(user2);
		when(dataSource.getConnection()).thenReturn(connection);

		mockMvc.perform(multipart("/api/loggedin/themes/1/ideas").file(mockThemeFile)
	...
	@Test
	public void viewThemesTest() throws Exception {
		List<Theme> themes = new ArrayList<>();
		themes.add(t);
		
		when(themeService.viewThemes()).thenReturn(themes);
	...
	@Test
	public void viewIdeasTest() throws Exception {
		List<Idea> ideas = new ArrayList<>();
		ideas.add(i);
		
		when(ideaService.viewIdeas(t.getThemeId())).thenReturn(ideas);
		
		mockMvc.perform(get("/api/themes/1/ideas")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}
	...
	public void downloadThemeFileTest() throws Exception {
		...
		mockMvc.perform(get("/api/loggedin/themes/1/download_file/1")
				.contentType(MediaType.APPLICATION_JSON)
				.header("Authorization", "Bearer " + this.generateJWTToken(user1)))
		.andExpect(status().isOk());
	}
